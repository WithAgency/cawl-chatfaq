...
# NOTE: avoid importing Django model classes at module import time (see get_anonymous_user_class below)
...
# Lazy, safe access to Django's AnonymousUser so importing this module
# doesn't require DJANGO_SETTINGS_MODULE to be set.
_ANON_CLASS = None

def get_anonymous_user_class():
    """
    Return Django's AnonymousUser class, importing it lazily. If Django is not
    configured or import fails, return a minimal fallback class so import-time
    behavior is safe in non-Django contexts.
    """
    global _ANON_CLASS
    if _ANON_CLASS is None:
        try:
            from django.contrib.auth.models import AnonymousUser as _Anon  # local import
            _ANON_CLASS = _Anon
        except Exception:
            # Fallback minimal class (keeps isinstance checks safe)
            class _FallbackAnonymousUser:
                pass

            _ANON_CLASS = _FallbackAnonymousUser
    return _ANON_CLASS
...
class AIConsumer(CustomAsyncConsumer, AsyncJsonWebsocketConsumer):
    ...
    async def is_auth(self, scope):
        return (
            self.scope.get("user")
            and not isinstance(self.scope["user"], get_anonymous_user_class())
